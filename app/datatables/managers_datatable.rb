class ManagersDatatable
  delegate :params, :h, :link_to, to: :@view
    ...
    def initialize(view, user)
      @view = view
      @current_user = user
      @source = user.isSuperAdmin? ?  Manager.where(:id.ne => user.id.to_s, :email.ne => user.email) : user.partner.managers.where(:id.ne => user.id.to_s, :email.ne => user.email)
    end
  ...
  def data
    extratos.map do |d|
      [
        ...
        @view.layout_opts(@current_user, d,"managers")
      ]
    end
  end
